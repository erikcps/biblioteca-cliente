...
export class FuseNavigationService
{
    ...
    // MODULES AVAILABLES
    private _modulesAvailables: any;
     ...
     */
    setModules(navigation) {
        const modules = [];
        if(navigation) {
            navigation.map(item => {
                item.children.map(items => {
                    items.children.map(module => {
                        modules.push(module.id);
                    })
                })
            })
        }
        this._modulesAvailables = modules;
    }

    getModules() {
        return this._modulesAvailables;
    }
}
